...
def load_layers(dir_lfr):
	...
	for i in range(0,2):
		print("layers[{0}][from_id_node] = {1}\nlayers[{0}][from_id_com] = {2}\nlayers[{0}][nb_coms] = {3}\nlayers[{0}][nb_links] = {4}\nlayers[{0}]['nb_nodes'] = {5}".format(i,layers[i]["from_id_node"],layers[i]["from_id_com"],layers[i]['nb_coms'],layers[i]['nb_links'],layers[i]['nb_nodes']))
...
def rewrite_nodes_and_coms(num, directory, from_node, from_com):
	...
	with open(filename_nodes, 'r') as file:
		for line in file:
			...
			new_id1 = str(int(float(line[0])) + from_node)
			new_id2 = str(int(float(line[1])) + from_node)
	...
	with open(filename_coms, 'r') as file:
		for line in file:
			...
			new_id1 = str(int(float(line[0])) + from_node)
			new_id2 = str(int(float(line[1])) + from_com)
...
def benchmark_lfr(num, directory, n, k, maxk, mu):
	...
	nb_com_layer = int(float(out))

	cmd = "wc -l %s/network.dat | cut -d\" \" -f1" % (dir_layer)
	process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = process.communicate()
	errcode = process.returncode
	nb_link_layer = int(float(out))
...
def create_cross_layer_communities(nb_layers, layers, alpha):	
	...
	for i in range(0, nb_layers-1):
		...
		while j < nb_cross_coms:
			...
			#print("To be made multilayer {0} {1}".format(index_coms_l1,index_coms_l2))
			...
			#pprint(cross_communities_l1_l2)
	...
	pprint(cross_communities)
	print("\nsingle layer communities in each layer:")
	pprint(intra_coms)
	print("\nnuber of single layer communities:")
	pprint(nb_intra_coms)
	print("\n========================================================\n")
...
def apply_p1_parameter(nb_layers, layers, couplings_links, cross_communities, p1):
	...
	for i in range(0, nb_layers-1):
		...
		# pprint("couplings_links,list_id_coms_layer1,list_id_coms_layer2,cross_communities")
		# pprint(couplings_links)
		# pprint(list_id_coms_layer1)
		# pprint(list_id_coms_layer2)
		# pprint(cross_communities)

		for id_com_l1, id_com_l2 in cross_communities:
			#print(id_com_l1,id_com_l2)
			if id_com_l1 in list_id_coms_layer1 and id_com_l2 in list_id_coms_layer2:
				list_cross_l1_l2.append((id_com_l1, id_com_l2))
				#pprint(list_cross_l1_l2)
		...
		for id_com_l1, id_com_l2 in list_cross_l1_l2:
			...
			# print("list_node_com_l1: ")
			# pprint(list_node_com_l1)
			# print("list_node_com_l2: ")
			# pprint(list_node_com_l2)

			nb_nodes_coupling_l1 = int(round(len(list_node_com_l1) * p1))
			nb_nodes_coupling_l2 = int(round(len(list_node_com_l2) * p1))
			# print("nb_nodes_coupling_l1: {0} \nnb_nodes_coupling_l2: {1}".format(nb_nodes_coupling_l1,nb_nodes_coupling_l2))

			sample_node_l1 = sample(list_node_com_l1, nb_nodes_coupling_l1)
			sample_node_l2 = sample(list_node_com_l2, nb_nodes_coupling_l2)
			# print("sample_node_l1: {0} \sample_node_l2: {1}".format(sample_node_l1,sample_node_l2))
			...
			while len(s_l1) > 0 or len(s_l2) > 0:
				...
				if len(s_l1) > 0:
					...
					# print("in if s_l2:",s_l2)
				elif len(s_l2) > 0:
					s_l1 = sample(sample_node_l1, min(len(s_l2), len(sample_node_l1)))
					# print("in else s_l1:",s_l1)
		print(nb_link_inside_cross_coms)
...
def apply_p2_parameter(nb_layers, layers, couplings_links, intra_coms, cross_communities, p2):
	...
	for i in range(0, nb_layers-1):
		...
		for id_com_cross_l1, id_com_cross_l2 in list_cross_l1_l2:
			...
			# pprint(layers[i]['com_nodes'][id_com_cross_l1])
			# pprint(layers[i+1]['com_nodes'][id_com_cross_l2])
		#print(len(list_node_l1),len(list_node_l2))

		#=============================================================================================
		# (a)	list_node_l1 and list_node_l2 contains all the nodes, i.e 100 nodes each
		# (b)	intra_coms is a dict whichcontains all single layer communities in each layer
		# (c)	list_cross_l1_l2 and cross_communities are same, they contains all the cross layer communities in tuple form
		# (d)	layers[i]['com_nodes'][id_com_l1] -> gives all the nodes in that community 'id_com_l1'
		# (e)	list_id_coms_layer1 and list_id_coms_layer2 contains nodes (1-100) and (101-200)
		#==============================================================================================
		for id_com_l1 in intra_coms[i]:
			#print("com id: ",id_com_l1)
			list_node_com_l1 = layers[i]['com_nodes'][id_com_l1]
			#print(list_node_com_l1)
			nb_nodes_coupling_l1 = int(round(len(list_node_com_l1) * p2))
			#print(len(list_node_com_l1),p2,nb_nodes_coupling_l1)
			
			sample_node_l1 = sample(list_node_com_l1, nb_nodes_coupling_l1)
			#print("sample node selected: ",sample_node_l1)
			...
			for j in range(len(sample_node_l1)):
				...
				#print("coupling: ",(sample_node_l1[j], list_node_l2[n_l2]))
				
				nb_link_outside_cross_coms += 1
		
		print("nb_link_outside_cross_coms",nb_link_outside_cross_coms)
		...
		for id_com_l2 in intra_coms[i+1]:
			...
			for j in range(len(sample_node_l2)):
				...
				#print("coupling: ",(list_node_l1[n_l1], sample_node_l2[j]))
				nb_link_outside_cross_coms += 1
		print("nb_link_outside_cross_coms",nb_link_outside_cross_coms)
...
def apply_p_parameter(nb_layers, layers, couplings_links, intra_coms, cross_communities, nb_link_inside_cross_coms, nb_link_outside_cross_coms, p):
	...
	for i in range(0, nb_layers-1):
		...
		pprint(list_com_node)
		# (com1 ,com2): {0: nodes of com1 connected to nodes of com 2, 1: com2 nodes}
		# {(5, 14): {0: set(['25', '36']), 1: set(['142.0'])},
		#  (8, 22): {0: set(['35', '81']), 1: set(['154.0', '171.0'])},
		#  (12, 25): {0: set(['7']), 1: set(['141.0'])}}
	...
	if p > p_temp:
		...
		for id_com_l1, id_com_l2 in list_com_node:
			print("len(list_com_node[(id_com_l1, id_com_l2)]): ",len(list_com_node[(id_com_l1, id_com_l2)]))
		...
		print("total_possible_nb_coupling",total_possible_nb_coupling)
		total_possible_nb_coupling -= nb_link_inside_cross_coms
		print("To add: ",total_possible_nb_coupling)
		...
		while p > p_temp and nb_new_coupling < total_possible_nb_coupling:
			...
			if (node_l1, node_l2) not in couplings_links[i_layer][i_layer+1]:
				...
				#print("coupling: ",(node_l1, node_l2))
				nb_new_coupling += 1
	...
	print(nb_link_inside_cross_coms,nb_link_outside_cross_coms,nb_new_coupling)
...
def main(argv):
	...
	print("total number of communities, nb_com = {0}\n".format(nb_com))
